...
const calculateRecommendedFood = dog => dog.recommendedFood = Math.round(dog.weight ** 0.75 * 28);

//* 1.
dogs.forEach(dog => calculateRecommendedFood(dog));
...
const createStr = arr => {
  return arr
    .join(' and ')
    .replace(',', ' and ')
    .concat('\'s dogs are eating too much.');
}

// console.log(createStr(ownersEatTooMuch));
// console.log(createStr(ownersEatTooLittle));
